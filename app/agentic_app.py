...
def display_research_results(result: Dict[str, Any]):
	...
	if result["success"]:
		...
		if result.get("adverse_media"):
			adv = result["adverse_media"]
			st.subheader("🚨 Adverse Media Screening")
			st.metric("Risk score", f"{adv['risk_score']}/10")
			st.write(f"Findings: {len(adv['adverse_findings'])}")
			for hit in adv["adverse_findings"]:
				st.markdown(f"- [{hit['headline']}]({hit['url']}) — *{hit['source']}*")
...
def main():
	...
	with st.sidebar:
		...
		include_adverse_media = st.checkbox(
			"🚨 Include Adverse Media",
			value=False,
			help="Check negative news, sanctions, lawsuits, etc."
		)
			...
			if company_name.strip():
				try:
					with st.spinner(f"🔍 Researching {company_name}..."):
						result = st.session_state.agent.research_business(company_name.strip(),
																		  location.strip(),include_adverse_media=include_adverse_media)
